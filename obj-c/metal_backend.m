...
@property(nonatomic, strong) id<MTLBuffer> LUTDataBuffer;
@property(nonatomic, strong) id<MTLBuffer> LUTScaleBuffer;
...
int metalUploadScene(metal_ctx *c_ctx, scene *sc) {
  ...
  @autoreleasepool {
    ...
    // LUTDataBuffer
    // LUTScaleBuffer
    int LUTDataBufferSize = sizeof(float) * sc->spec_model->res *
                            sc->spec_model->res * sc->spec_model->res * 3 *
                            RGB2SPEC_N_COEFFS;
    int LUTScaleBufferSize = sc->spec_model->res * sizeof(float);

    id<MTLBuffer> LUTDataBuffer =
        [device newBufferWithBytes:sc->spec_model->data
                            length:LUTDataBufferSize
                           options:MTLResourceStorageModeShared];
    id<MTLBuffer> LUTScaleBuffer =
        [device newBufferWithBytes:sc->spec_model->scale
                            length:LUTScaleBufferSize
                           options:MTLResourceStorageModeShared];

    // id<MTLBuffer> modelBuffer =
    //     [device newBufferWithBytes:sc->spec_model
    //                         length:modelBufferSize
    //                        options:MTLResourceStorageModeShared];
    ...
    ctx.LUTDataBuffer = LUTDataBuffer;
    ctx.LUTScaleBuffer = LUTScaleBuffer;
